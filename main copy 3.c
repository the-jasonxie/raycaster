...
int main(int argc, const char * argv[]) {
    ...
    while (ProgramRunning)
    {
        ...
        // Calculate the two points on the camera plane based on the angles
        float CameraPlaneX_1 = PlayerX + 1 * DirectionX + cosf(DirectionAbsoluteRadian + ((FieldOfView * M_PI / 180) / 2));
        float CameraPlaneY_1 = PlayerY + 1 * DirectionY + sinf(DirectionAbsoluteRadian + ((FieldOfView * M_PI / 180) / 2));
        float CameraPlaneX_2 = PlayerX + 1 * DirectionX + cosf(DirectionAbsoluteRadian - ((FieldOfView * M_PI / 180) / 2));
        float CameraPlaneY_2 = PlayerY + 1 * DirectionY + sinf(DirectionAbsoluteRadian - ((FieldOfView * M_PI / 180) / 2));
        
        float CameraIncrementX = (CameraPlaneX_2 - CameraPlaneX_1) / ScreenWidth;
        float CameraIncrementY = (CameraPlaneY_2 - CameraPlaneY_1) / ScreenWidth;
        float y = CameraPlaneY_2;
        float x = CameraPlaneX_2;
        
        float angle;
        float Value;
        
        for (int Pixel = ScreenWidth; Pixel > 0; Pixel--)
        {
            angle = atan2((y - PlayerY), (x - PlayerX));
            // printf("%f\n", angle);
            
            // get point of intersection
            // get distance from intersection to cameraplane
            
            Value = CalculateRay(1, angle, PlayerX, PlayerY, DirectionX, DirectionY);
            
            if (Value > FOGDISTANCE)
            {
                float Fog = 200 * sinf(3 * M_PI);
                SDL_SetRenderDrawColor(Renderer, (120), (120), (120), 120);
                SDL_RenderDrawLine(Renderer, Pixel, ScreenHeight / 2 + Fog, Pixel, ScreenHeight / 2 - Fog);
            }
            else
            {
            ...
            // Value = sqrtf((ClosestCoordinate[0] - x) * (ClosestCoordinate[0] - x) + (ClosestCoordinate[1] - y) * (ClosestCoordinate[1] - y));
            // Value = Value * cosf(angle - DirectionAbsoluteRadian);
            ...
            for (int i = 0; i < NumberOfSprites; i++)
            {
                ...
                if (fabs(Sprites[i].AngleToPlayer - angle) < 0.001) // Tolerance level.
                {
                    if (i == 0)
                    {
                        printf("%f\n", angle);
                    ...
                    if (Sprites[i].DistanceToPlayer < Sprites[i].PlayerToWallDistance && Sprites[i].DistanceToPlayer < FOGDISTANCE)
                    {
                        SDL_SetRenderDrawColor(Renderer, (255), (255), (255), 255);
                        SDL_RenderDrawLine(Renderer, Pixel, ((ScreenHeight / 2) - (ScreenHeight / 2) / Value), Pixel, ((ScreenHeight / 2) + (ScreenHeight / 2) / Value));
...
float CalculateRay(int ObjectIdentifying, float RayRadian, float PlayerX, float PlayerY, float DirectionX, float DirectionY)
{
    ...
    ClosestCoordinate[0] = 0;
    ClosestCoordinate[1] = 0;
    ...
    */
    float RayX = DirectionX + cosf(RayRadian);
    float RayY = DirectionY + sinf(RayRadian);
        ...
        for (int i = 0; i < MAXCHECK; i++)
        {
            ...
            if (Map[(int) xTraverse][(int) yTraverse] == ObjectIdentifying)
            {
                ...
                if (DistanceFromStart < LowestDistance || LowestDistance < 0.001)
                {
                    ...
                    ClosestCoordinate[0] = xTraverse;
                    ClosestCoordinate[1] = yTraverse;
                }
                break;
            }
